...
const BASE_URL = "http://localhost:9080"; // 🔁 Update this for production or Netlify
...
export const addNewSite = async (siteData) => {
  try {
    const response = await fetch("http://localhost:9080/siteMasterData", {
      method: "POST",
      headers: {
...
export const addCameraReport = async (report) => {
  const response = await fetch("http://localhost:9080/camera-reports", {
    method: "POST",
